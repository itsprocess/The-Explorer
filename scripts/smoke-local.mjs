...
const shared=JSON.parse(workshop.pass2Prompt.input).connected_cells.find(n=>n.x===0&&n.y===0).package;
assert.equal(shared.shared_exit.direction,'east');
assert.ok(shared.shared_exit.description.length>5);
assert.ok(shared.exits.some(e=>e.description===shared.shared_exit.description));

...
const leaked=[...tracked,...walk('dist'),...walk('runtime'),'app.js'].filter(p=>readFileSync(p).includes(Buffer.from(key)));
if(leaked.length)throw Error('Secret found in '+leaked.join(', '));
console.log('Verified server key absent from tracked source, runtime, and all compiled files.');

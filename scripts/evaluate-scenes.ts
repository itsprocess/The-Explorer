...
const cases=[{name:'ordinary',id:null},{name:'forest',id:'vegetation.forest'},{name:'settlement',id:'civilization.settlement'},{name:'treasure',id:'encounters.treasure'}];
const results=[];
for(const sample of cases){
 let found:{x:number;y:number}|undefined;
 for(let i=1;i<100000;i++){
  const x=i*8,y=i%179-89,rs=deriveRatings('scene-evaluation',x,y);
  if(sample.id?rs.find(r=>r.id===sample.id)!.value>0:!rs.some(r=>r.kind==='feature'&&r.value>0)){found={x,y};break;}
 }

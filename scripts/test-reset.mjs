...
data('reset','--scope','world','--confirm','RESET');
status=JSON.parse(data('status'));assert.equal(status.counts.characters,1);assert.equal(status.counts.character_credentials,1);assert.equal(status.counts.character_sessions,1);assert.equal(status.counts.packages,0);assert.equal(status.counts.visits,0);assert.equal(status.counts.claims,0);
run(['node_modules/wrangler/bin/wrangler.js','d1','execute','DB','--local','--config','wrangler.local.json','--persist-to','outputs/reset-test-db','--command',"INSERT INTO packages(key,kind,value,updated) VALUES ('test','cell','{}',1)"]);
...
data('migrate');
console.log('Passed isolated resets: confirmation required, world reset preserves identities/sessions, character-only retains world, full reset clears all, schema survives.');

...
test('building and constructed basement omit groundcover without mutating world fields',()=>{
 for(const underground of [0,1]){const c=cell(1,underground);assert.ok(!environmentPromptFields(c).some(f=>f.id==='biome.groundcover'));assert.ok(!settingInput([c],[]).definitions.some(d=>d.name==='Groundcover'));assert.equal(c.fieldwork[0].value,.9);assert.match(sceneInstructionsFor(c),/architectural floor/);assert.equal(settingCachePrefix(c),'setting-interior-v3:');}
});
test('outdoors and natural caves retain groundcover and existing cache keys',()=>{
 for(const underground of [0,1]){const c=cell(0,underground);assert.ok(environmentPromptFields(c).some(f=>f.id==='biome.groundcover'));assert.equal(settingCachePrefix(c),'setting-v2:');assert.doesNotMatch(sceneInstructionsFor(c),/architectural floor/);}
});

...
test('runtime and noise-app default contain the identical complete valid stack',()=>{
 ...
 for(const id of ['biome.off_ground','biome.weather_severity','variation.vibrance','civilization.devoutness','occurrences.relic'])assert.equal(p.variables.filter(v=>v.appName===id).length,1);
});
...
test('sky, weather and vibrance escalate radially, preserve zero, and leave unselected biomes unchanged',()=>{
 const p=parseProject(JSON.stringify(baseline));p.variables=p.variables.filter(v=>['biome.off_ground','biome.weather_severity','variation.vibrance','natural.elevation'].includes(v.appName));p.civilizationObeysTraversal=false;

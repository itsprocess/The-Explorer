...
test('lake candidates increase 5x without changing basin sizes; masks remain last',()=>{
 const p=parseProject(JSON.stringify(baseline)),v=p.variables.find(v=>v.appName==='biome.lakes_ponds')!;
 const patches=v.layers.filter(l=>l.source==='patches');assert.equal(patches.length,6);
 assert.ok(Math.abs(patches.reduce((n,l)=>n+l.patchChance!,0)/patches[0].patchChance!-5)<1e-9);
...
test('void pockets block movement and stay excluded from ocean',()=>{
 ...
 assert.ok(Math.abs(v.layers[0].scaleX-150)<1e-9);
...
test('requested reset clears progress, preserves credentials/session/trait, and queues old image removal',()=>{
 ...
 db.exec(readFileSync(new URL('../drizzle/0031_reset_encounter_balance.sql',import.meta.url),'utf8'));

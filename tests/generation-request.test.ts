...
test('generation waits preserve movement idempotency, switch authenticated retries to GET, and reject other conflicts',async t=>{
 ...
 assert.equal(requests.length,2);assert.equal(requests[0].init?.body,requests[1].init?.body);assert.equal(requests[0].init?.method,requests[1].init?.method);
 pending=true;requests.length=0;
 await generationRequest('/api/character',{method:'POST',body:'credentials'},'/api/game');
 assert.equal(requests[1].url,'/api/game');assert.equal(requests[1].init?.body,undefined);assert.equal(requests[1].init?.method,undefined);

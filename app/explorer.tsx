...
import TraitDisplay,{StateChanges} from './trait-display';
...
export default function Explorer(){
 ...
 const pendingFeedback=useRef<any>(null),warmed=useRef(new Set<string>());
 ...
 async function retryGeneration(forceResume=false){
  ...
  try{if(forceResume||/API credit|rate limited/.test(message)){const r=await fetch('/api/generation/retry',{method:'POST'});if(!r.ok){const d:any=await r.json();throw Error(d.error);}}await load();setImageRetry(n=>n+1);if(tab==='dev')await workshop();}
  catch(e:any){setError(e.message);}finally{setBusy(false);}
 }
 ...
 async function historyPage(offset:number){try{const d:any=await generationRequest('/api/game?offset='+offset);setState(d);}catch(e:any){setError(e.message);}}
 ...
 function warmDirection(direction:string){if(!c||busy||c.pendingTransport)return;const key=locationKey+':'+direction;if(warmed.current.has(key))return;if(warmed.current.size>100)warmed.current.clear();warmed.current.add(key);void generationRequest('/api/preload',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({x:c.x,y:c.y,direction,stage:'all',priority:'next'})}).catch(()=>{});}
 ...
 return <main>{tab==='explore'&&!concealed&&feedback.popup}{tab==='explore'&&!concealed&&c?.pendingTransport&&<TeleportDialog key={c.pendingTransport.token} pending={c.pendingTransport} onConfirm={()=>act('teleport',{token:c.pendingTransport.token})}/>}{c?.alive&&cell&&!busy&&!error&&!imageFailure&&<NeighborPreloader key={locationKey} x={c.x} y={c.y} connections={state.connections} imageReady={readyImage===locationKey} pendingTransport={c.pendingTransport?.token}/>}
  <header className="masthead"><a className="brand" href="/">The Explorer</a><nav aria-label="Main" inert={concealed&&!error&&!imageFailure}><button className={tab==='explore'?'active':''} onClick={()=>{if(tab!=='explore')setReadyImage('');setTab('explore');}}>Explore</button>{c&&<button className={tab==='profile'?'active':''} onClick={()=>setTab('profile')}>Profile</button>}<button className={tab==='dev'?'active':''} onClick={workshop}>Dev</button></nav>{feedback.soundButton}<About/>{c&&<button disabled={busy} onClick={()=>auth('logout')}>Log out</button>}</header>
  {error&&<div className="error" role="alert">{error} <button disabled={busy} onClick={()=>retryGeneration()}>Retry generation</button>{error.includes("Sign in with ChatGPT")&&<> <a href="/signin-with-chatgpt?return_to=/">Sign in</a></>}</div>}

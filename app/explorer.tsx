 ...
 return <main>{feedback.popup}
  ...
  {!state&&!error?<p className="loading">Loading…</p>:!c&&tab!=='dev'?<form className="auth" onSubmit={e=>{e.preventDefault();auth('login');}}>
   ...
   <section className="reading-pane" aria-busy={busy}>
    {tab==='explore'&&<><h1>{scene?.title??'Loading…'}</h1><div className="region-line">{cell?.regions?.map((r:any)=><span key={r.id}><span className="region-label">{r.kind[0].toUpperCase()+r.kind.slice(1)}: </span>{r.name}</span>)}</div>{cell&&<LocationImage key={c.id+":"+c.x+":"+c.y} x={c.x} y={c.y} title={scene?.title} savedUrl={cell.imageUrl}/>}<div className="scene">{scene?.description?.split('\n').filter(Boolean).map((p:string,i:number)=><p key={i}>{p}</p>)}</div><ul className="exits">{scene?.exits?.map((e:any)=><li key={e.direction}><strong>{e.direction[0].toUpperCase()+e.direction.slice(1)}</strong> — {e.description} <span className="glimpse">{e.glimpse}</span></li>)}</ul>{scene?.blocked?.length>0&&<ul className="blocked">{scene.blocked.map((b:any)=><li key={b.direction}><strong>{b.direction[0].toUpperCase()+b.direction.slice(1)}</strong> — {b.reason}</li>)}</ul>}{state?.lastEvent&&state.lastEvent.kind!=="arrival"&&<div className={'encounter '+(!c?.alive?'death':'')}><p>{state.lastEvent.text}</p>{state.lastEvent.newBadge&&<small>Badge: {state.lastEvent.newBadge}</small>}</div>}{cell&&<a href={'/cell/'+c.x+'/'+c.y}>View-only location ↗</a>}</>}

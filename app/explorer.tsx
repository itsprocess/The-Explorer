...
export default function Explorer(){
 ...
 return <main>{!concealed&&feedback.popup}{!concealed&&c?.pendingTransport&&<TeleportDialog key={c.pendingTransport.token} pending={c.pendingTransport} onConfirm={()=>act('teleport',{token:c.pendingTransport.token})}/>}{c?.alive&&cell&&<NeighborPreloader key={locationKey} x={c.x} y={c.y} connections={state.connections} imageReady={readyImage===locationKey} pendingTransport={c.pendingTransport?.token}/>}
  <header className="masthead"><a className="brand" href="/">The Explorer</a><nav aria-label="Main" inert={concealed}><button className={tab==='explore'?'active':''} onClick={()=>{if(tab!=='explore')setReadyImage('');setTab('explore');}}>Explore</button>{c&&<button className={tab==='profile'?'active':''} onClick={()=>setTab('profile')}>Profile</button>}<button className={tab==='dev'?'active':''} onClick={workshop}>Dev</button></nav>{feedback.soundButton}<About/>{c&&<button disabled={busy} onClick={()=>auth('logout')}>Log out</button>}</header>

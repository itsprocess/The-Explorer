...
export default function Explorer(){
 ...
 return <main>
  ...
  {busy&&<div className="working" role="status">Loading…</div>}
  {!state&&!error?<p className="loading">Loading…</p>:!c&&tab!=='dev'?<form className="auth" onSubmit={e=>{e.preventDefault();auth('login');}}>
   ...
   <section className="reading-pane">
    {tab==='explore'&&<><h1>{scene?.title??'Loading…'}</h1><div className="region-line">{cell?.regions?.map((r:any)=><span key={r.id}>{r.name}</span>)}</div><div className="scene">{scene?.description?.split('\n').filter(Boolean).map((p:string,i:number)=><p key={i}>{p}</p>)}</div>{state?.lastEvent&&<div className={'encounter '+(!c?.alive?'death':'')}><p>{state.lastEvent.text}</p>{state.lastEvent.newBadge&&<small>Badge: {state.lastEvent.newBadge}</small>}</div>}{cell&&<a href={'/cell/'+c.x+'/'+c.y}>Share location ↗</a>}</>}
    {tab==='profile'&&c&&<><h1>{c.name}</h1><div className="stats"><div><span>Deaths</span><strong>{c.deaths}</strong></div><div><span>Furthest distance</span><strong>{c.furthest.toFixed(1)}</strong></div><div><span>Badges</span><strong>{state.badges?.length??0}</strong></div></div><div className="badges">{state.badges?.map((b:any)=><article className="badge" key={b.id}><strong>{b.title}</strong><p>{b.description}</p></article>)}</div><h2>History</h2>{state.history?.map((h:any)=><article className="encounter" key={h.id}><a href={'/cell/'+h.x+'/'+h.y}>{h.x}, {h.y}</a><p>{h.text}</p></article>)}<div className="buttons">{state.historyOffset>0&&<button onClick={()=>historyPage(state.historyOffset-25)}>Newer</button>}{state.historyHasMore&&<button onClick={()=>historyPage(state.historyOffset+25)}>Older</button>}<a href={'/profile/'+c.id}>Share profile ↗</a></div></>}
    {tab==='dev'&&<><h1>Generation</h1>{inspect?<>{Object.entries({pass1Prompt:'Pass 1 prompt',pass1Result:'Pass 1 result',pass2Prompt:'Pass 2 prompt',pass2Result:'Pass 2 result'}).map(([k,label])=><details key={k}><summary>{label}</summary><pre>{JSON.stringify(inspect[k],null,2)??'Not generated'}</pre></details>)}<details><summary>Ratings</summary><div className="recipe-list">{inspect.ratings?.map((r:any)=><div key={r.id}><span>{r.id}</span><meter min={0} max={1} value={r.value}/><b>{r.value.toFixed(4)}</b></div>)}</div></details></>:<p>Loading…</p>}</>}
   </section>
  </div>}
 </main>;
}

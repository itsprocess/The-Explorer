...
export default function Explorer(){
 ...
 async function act(action:string,extra:Record<string,unknown>={}){
  ...
  try{const d:any=await generationRequest('/api/game',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(body)});retryAction.current=null;pendingFeedback.current={before:state,after:d};setState(d);setInspect(null);return d;}
 ...
 return <main>{tab==='explore'&&!concealed&&feedback.popup}{tab==='explore'&&!concealed&&c?.pendingTransport&&<TeleportDialog key={c.pendingTransport.token} pending={c.pendingTransport} onConfirm={()=>act('teleport',{token:c.pendingTransport.token})}/>}
  <header className="masthead"><a className="brand" href="/">The Explorer</a><nav aria-label="Main" inert={concealed&&!error&&!imageFailure}><button className={tab==='explore'?'active':''} onClick={()=>{if(tab!=='explore')setReadyImage('');setTab('explore');}}>Explore</button>{c&&<button className={tab==='profile'?'active':''} onClick={()=>setTab('profile')}>Profile</button>}{(!state||state.canInspect)&&<button className={tab==='dev'?'active':''} onClick={workshop}>Dev</button>}</nav>{feedback.soundButton}<About/>{c&&<button disabled={busy} onClick={()=>auth('logout')}>Log out</button>}</header>

"use client";
import DeathRecords from './death-records';
...
import ExplorersBrowser from './explorers-browser';
...
export default function Explorer(){
 ...
 const [waiting,setWaiting]=useState(''),[devPassword,setDevPassword]=useState(''),[devError,setDevError]=useState('');
 const [warpX,setWarpX]=useState('0'),[warpY,setWarpY]=useState('0');
 const [authMode,setAuthMode]=useState<'login'|'create'>('login');
 const [definingTrait,setDefiningTrait]=useState<string>('');
 const [state,setState]=useState<any>(null),[busy,setBusy]=useState(false),[error,setError]=useState(''),[tab,setTab]=useState('explore'),[historyQuery,setHistoryQuery]=useState(''),[name,setName]=useState(''),[password,setPassword]=useState(''),[inspect,setInspect]=useState<any>(null);
 ...
 async function act(action:string,extra:Record<string,unknown>={}){
  if(action==='move'&&(state?.character?.pendingTransport||state?.character?.pendingOption)){setWaiting('');setTab('explore');feedback.clear();return state;}
 ...
 async function workshop(unlocked=state?.canInspect){setTab('dev');setInspect(null);if(!unlocked)return;try{const r=await fetch(appPath('/api/workshop?x=')+(state?.character?.x??0)+'&y='+(state?.character?.y??0));const d:any=await r.json();if(!r.ok)throw Error(d.error);setInspect(d);}catch(e:any){setError(e.message);}}
 async function unlockDev(){setBusy(true);setDevError('');try{const r=await fetch(appPath('/api/dev/unlock'),{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({password:devPassword})});const d:any=await r.json();if(!r.ok)throw Error(d.error);setDevPassword('');setState((s:any)=>({...s,canInspect:true}));await workshop(true);}catch(e:any){setDevError(e.message);}finally{setBusy(false);}}
 ...
 const pendingKey=c?.pendingTransport?.token||c?.pendingOption?.visit||'';
 ...
 const giveUp=()=>{if(window.confirm('End this life and return to the sanctuary? Temporary possessions and statuses will be lost.'))void act('give_up');};
 return <main>{state?.needsIntro&&cell&&!busy&&<WelcomeDialog busy={busy} onContinue={()=>act('intro')}/>} {tab==='explore'&&!concealed&&feedback.popup}{tab==='explore'&&!concealed&&!feedback.popup&&c?.pendingTransport&&waiting!==pendingKey&&<TeleportDialog onWait={()=>setWaiting(pendingKey)} busy={busy} error={error} key={c.pendingTransport.token} pending={c.pendingTransport} setup={state.encounterSetup} onConfirm={()=>act('teleport',{token:c.pendingTransport.token})}/>}
  {c&&!c.definingTrait&&<section className="auth"><TraitPicker value={definingTrait} onChange={setDefiningTrait} disabled={busy}/><button disabled={busy||!definingTrait} onClick={()=>act('trait',{trait:definingTrait})}>Choose trait</button></section>}{tab==='explore'&&!feedback.popup&&c?.pendingOption&&waiting!==pendingKey&&state?.optionChoices&&<OptionDialog onWait={()=>setWaiting(pendingKey)} key={c.pendingOption.visit} choices={state.optionChoices} setup={state.encounterSetup} busy={busy} error={error} onChoose={choice=>act('option',{choice,visit:c.pendingOption.visit})}/>}<header className="masthead"><a className="brand" href={appPath('/')}>The Explorer</a><nav aria-label="Main" inert={concealed&&!error&&!imageFailure}><button className={tab==='explore'?'active':''} onClick={()=>{if(tab!=='explore')setReadyImage('');setTab('explore');}}>Explore</button>{c&&<button className={tab==='profile'?'active':''} onClick={()=>setTab('profile')}>Profile</button>}{c&&<><button className={tab==='history'?'active':''} onClick={()=>{setHistoryQuery('');setTab('history');}}>History</button><button className={tab==='explorers'?'active':''} onClick={()=>setTab('explorers')}>Explorers</button></>}{(state?.canAccessDev||state?.canInspect)&&<button className={tab==='dev'?'active':''} onClick={()=>workshop()}>Dev</button>}</nav>{feedback.soundButton}<About/>{c&&<button disabled={busy} onClick={()=>auth('logout')}>Log out</button>}</header>{state?.devMode&&<div className="dev-travel-banner">Dev travel: discoveries and your real run are unchanged. <button disabled={busy} onClick={()=>act('dev_exit')}>Return to normal</button></div>}
  ...
  {!state&&!error?<div className="workspace"><section className="reading-pane"><div className="image-placeholder"><span role="status"><span className="spinner" aria-hidden="true"/>Loading…</span></div></section></div>:!c&&tab!=='dev'?<form className="auth" onSubmit={e=>{e.preventDefault();auth(authMode);}}>
   <div className="auth-mode" role="group" aria-label="Character access"><button type="button" aria-pressed={authMode==='login'} disabled={busy} onClick={()=>{setAuthMode('login');setError('');}}>Log in</button><button type="button" aria-pressed={authMode==='create'} disabled={busy} onClick={()=>{setAuthMode('create');setError('');}}>Create character</button></div><h1>{authMode==='login'?'Welcome back':'Create your character'}</h1><label htmlFor="name">Name</label><input id="name" autoComplete="username" minLength={2} maxLength={40} required value={name} onChange={e=>setName(e.target.value)}/>
   <label htmlFor="password">Password</label><input id="password" type="password" autoComplete={authMode==='login'?'current-password':'new-password'} minLength={8} maxLength={128} required value={password} onChange={e=>setPassword(e.target.value)}/>
   {authMode==='create'&&<TraitPicker value={definingTrait} onChange={setDefiningTrait} disabled={busy}/>}<button className="primary" disabled={busy||(authMode==='create'&&!definingTrait)}>{busy?'Please wait…':authMode==='login'?'Log in':'Create character'}</button>
   ...
   {c&&<aside className="atlas"><div className="map" aria-label="Nearby cells">{Array.from({length:121},(_,i)=>{const x=c.x+i%11-5,y=c.y+Math.floor(i/11)-5;const m=state?.map?.find((p:any)=>p.x===x&&p.y===y);const Tag=m?.visited?'button':'span';return <Tag key={i} onClick={m?.visited?()=>{setHistoryQuery(x+', '+y);setTab('history');}:undefined} aria-label={m?.visited?'View your history at '+x+', '+y:undefined} title={x+', '+y+' · '+(m?.visited?'Visited by you':m?.generated?'Discovered by others':'Unexplored')} className={['map-cell',m?.exists?'open':'',m?.generated?'known':'',m?.visited?'visited':'',i===60?'current':''].join(' ')}>{m?.icon&&<b className="map-tag" aria-label={tileIcons.find(t=>t.id===m.icon)?.label}>{tileIcons.find(t=>t.id===m.icon)?.icon}</b>}{i===60?<i className="map-dot"/>:x===0&&y===0?<i className="map-home"/>:null}</Tag>;})}</div><div className="map-legend"><span>Unexplored</span><span>Others</span><span>You</span></div><div className="coordinates">{c.x}, {c.y}</div><TileMarker x={c.x} y={c.y}/>
   ...
   <section className="reading-pane" aria-busy={busy}>
    {tab==='explore'&&<><div className="location-heading"><h1>{scene?.title??''}</h1>{state?.tileMemory?.discoverer&&<small className="discovery-credit">Discovered by <a href={appPath('/profile/')+state.tileMemory.discoverer.id}>{state.tileMemory.discoverer.name}</a></small>}{state?.tileMemory?.relics?.length>0&&<div className="relic-records"><strong>Relics uncovered here</strong>{state.tileMemory.relics.map((r:any)=><p key={r.id}>{r.text} <a href={appPath('/profile/')+r.character}>{r.name??'Explorer'}</a></p>)}</div>}<div className="region-line">{cell?.regions?.map((r:any)=><span key={r.id}><span className="region-label">{r.kind[0].toUpperCase()+r.kind.slice(1)}: </span>{r.name}</span>)}</div></div>{cell&&<LocationImage key={c.id+":"+c.x+":"+c.y} x={c.x} y={c.y} title={scene?.title} savedUrl={cell.imageUrl} pending={busy} onReady={markImageReady} onFailure={markImageError} retry={imageRetry}/>}<div className="scene">{scene?.description?.split('\n').filter(Boolean).map((p:string,i:number)=><p key={i}>{p}</p>)}</div>{state?.tileMemory?.imprint&&<div className="scene shared-imprint"><small>A lasting trace</small><p>{state.tileMemory.imprint}</p></div>}<Passages exits={scene?.exits??[]} blocked={scene?.blocked??[]} onMove={c?.alive?direction=>act('move',{direction}):undefined} disabled={busy}/>{state?.lastEvent&&state.lastEvent.kind!=="arrival"&&<div className={'encounter player-record '+(!c?.alive?'death':'')}><PlayerMark/><p>{state.lastEvent.text}</p><PublicStateChanges kind={state.lastEvent.kind} changes={state.lastEvent.stateChanges}/>{state.lastEvent.newBadge&&<small>Badge: {state.lastEvent.newBadge}</small>}</div>}{cell&&<a href={appPath('/cell/')+c.x+'/'+c.y}>Share this location ↗</a>}</>}
    {tab==='explore'&&state?.lock&&<section className="personal-lock"><h2>{state.lock.kind==='tile'?'Locked location':'Locked discovery'}</h2><p>Requires a {state.lock.rarity} or higher key. Opens for you permanently, including through death.</p><button disabled={busy||!state.lock.canUnlock} onClick={()=>act('unlock')}>{state.lock.keyName?'Unlock — uses '+state.lock.keyName:'Find a matching key'}</button></section>}
    {tab==='history'&&c&&<HistoryBrowser key={c.id+historyQuery} character={c.id} initialQuery={historyQuery}/>}
    {tab==='explorers'&&c&&<ExplorersBrowser/>}
    {tab==='profile'&&c&&<><h1>{c.name}</h1>{c.alive&&<button className="give-up" disabled={busy} onClick={giveUp}>Give up this life</button>}<p>Defining trait: {c.definingTrait??'Not yet selected'}</p><h2>Badges</h2>{!state?.badges?.length&&<p className="muted">No badges yet.</p>}<div className="badges">{state.badges?.map((b:any)=><a className="badge" key={b.id} href={badgeLink(c.id,b.id)}><strong>{b.title}</strong><p>{b.description}</p></a>)}</div><DeathRecords key={c.id+":"+c.deaths} character={c.id} count={c.deaths}/><ProgressStats character={c} badges={state.badges?.length??0}/><AffiliationDisplay standings={state.standings}/><TraitDisplay traits={state.traits} character={c.id}/><a href={appPath('/profile/')+c.id}>Share profile ↗</a></>}
    {tab==='dev'&&!state?.canInspect&&<form className="dev-unlock" onSubmit={e=>{e.preventDefault();unlockDev();}}><h1>Unlock Dev</h1><p>Enter the Dev password once for this character login session.</p>{!c?<p>Log in to your character first.</p>:<><label htmlFor="dev-password">Dev password</label><input id="dev-password" type="password" autoComplete="off" required maxLength={128} value={devPassword} onChange={e=>setDevPassword(e.target.value)}/><button className="primary" disabled={busy}>Unlock</button></>}{devError&&<p role="alert">{devError}</p>}</form>}{tab==='dev'&&state?.canInspect&&<><h1>Generation</h1><section className="dev-controls" aria-label="Developer travel and images">{tab==='dev'&&state?.canInspect&&<form className="warp-controls" onSubmit={e=>{e.preventDefault();act('dev_warp',{x:Number(warpX),y:Number(warpY)});}}><label><span>X coordinate</span><input type="number" step="1" required value={warpX} onChange={e=>setWarpX(e.target.value)}/></label><label><span>Y coordinate</span><input type="number" step="1" required value={warpY} onChange={e=>setWarpY(e.target.value)}/></label><button disabled={busy||!c}>Warp without discovery</button></form>}{tab==='dev'&&state?.canInspect&&c&&<button disabled={busy} onClick={async()=>{setBusy(true);setError('');try{const result:any=await generationRequest(appPath('/api/image'),{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({x:c.x,y:c.y,force:true})});setState((old:any)=>({...old,cell:{...old.cell,imageUrl:result.url}}));}catch(e){setError((e as Error).message);}finally{setBusy(false);}}}>Regenerate current image</button>}</section>{inspect?<><TokenUsagePanel data={inspect.usage}/>{inspect.providerPause&&<div className="error"><p>{inspect.providerPause.message}</p><button disabled={busy} onClick={()=>retryGeneration(true)}>Retry generation</button></div>}{inspect.diagnostic&&<details><summary>Generation diagnostics</summary><pre>{JSON.stringify(JSON.parse(inspect.diagnostic.value),null,2)}</pre></details>}{Object.entries({pass1Prompt:'Local preparation (no tokens)',pass2Prompt:'Scene prompt',pass2Result:'Scene result',imagePackage:'Image generation'}).map(([k,label])=><details key={k}><summary>{label}</summary><pre>{JSON.stringify(inspect[k],null,2)??'Not generated'}</pre></details>)}<h2>{inspect.context?.biome}</h2><div className="field-list">{inspect.ratings?.filter((r:any)=>r.kind==="feature"&&r.value>0).map((r:any)=><article key={r.id}><strong>{r.name}</strong><span>{r.value===1?"Present":Math.round(r.value*100)+"% strength"}</span><small>{r.recipe}</small></article>)}</div>{!inspect.ratings?.some((r:any)=>r.kind==="feature"&&r.value>0)&&<p>No special features.</p>}<details><summary>Baseline conditions</summary><div className="field-list">{inspect.ratings?.filter((r:any)=>r.kind==="baseline").map((r:any)=><article key={r.id}><strong>{r.name}</strong><span>{Math.round(r.value*100)}% · {r.low} → {r.high}</span><small>{r.recipe}</small></article>)}</div></details><details><summary>Absent features ({inspect.ratings?.filter((r:any)=>r.kind==="feature"&&r.value===0).length})</summary><div className="field-list">{inspect.ratings?.filter((r:any)=>r.kind==="feature"&&r.value===0).map((r:any)=><article key={r.id}><strong>{r.name}</strong><span>Absent</span><small>{r.recipe}</small></article>)}</div></details></>:null}</>}
   </section>

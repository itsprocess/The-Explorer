...
import {readPackage,db,namespace} from '../../../../lib/server';
...
export default async function CellPage({params}:{params:Promise<{x:string;y:string}>}){
 ...
 const cell=discovered?(await readPackage<CellPackage>(cellKey(x,y))??await readPackage<CellPackage>(namespace()+'dev-cell:'+x+':'+y)):null;

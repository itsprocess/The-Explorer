...
export default async function CellPage({params}:{params:Promise<{x:string;y:string}>}){
 ...
 return <main className="public-page"><a href="/">← The Explorer</a><div className="eyebrow" style={{marginTop:40}}>{x} / {y}</div><h1>{cell.scene.title}</h1><div className="region-line">{cell.regions.map(r=><span key={r.id}>{r.name}</span>)}</div><div className="rule"/>{cell.scene.description.split('\n').filter(Boolean).map((p,i)=><p key={i}>{p}</p>)}<ul className="exits">{cell.scene.exits.map(e=><li key={e.direction}><strong>{e.direction[0].toUpperCase()+e.direction.slice(1)}</strong> — {e.description}</li>)}</ul><h2>Visits</h2>{marks.map((m,i)=><article className="encounter" key={i}><a href={'/profile/'+m.character}>{JSON.parse(m.character_value).name}</a><p>{JSON.parse(m.value).event.text}</p></article>)}</main>;
}

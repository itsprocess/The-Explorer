...
export default function LocationImage({x,y,title,savedUrl,pending,onReady}:{x:number;y:number;title:string;savedUrl?:string|null;pending?:boolean;onReady?:()=>void}){
 ...
 useEffect(()=>{const control=new AbortController();setUrl(savedUrl||'');setLoaded(false);setError('');
  ...
  if(!savedUrl||attempt)void generationRequest('/api/image',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({x,y})},undefined,{signal:control.signal}).then(d=>{if(!control.signal.aborted)setUrl(d.url+(d.url.includes('?')?'&':'?')+'delivery='+attempt);}).catch(e=>{if(!control.signal.aborted)setError(e.message);});

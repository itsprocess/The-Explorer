...
export default async function ProfilePage({params,searchParams}:{params:Promise<{id:string}>;searchParams:Promise<{page?:string}>}){
 ...
 return <main className="public-page"><a href="/">← The Explorer</a><div className="eyebrow" style={{marginTop:40}}>A LIFE IN MANY CHAPTERS</div><h1>{c.name}</h1><div className="stats"><div><span>Total deaths</span><strong>{c.deaths}</strong></div><div><span>Furthest distance</span><strong>{c.furthest.toFixed(1)}</strong></div><div><span>Distinctions</span><strong>{c.badges.length}</strong></div></div><div className="badges">{c.badges.map(b=><article className="badge" key={b.id}><strong>{b.title}</strong><span>{b.description}</span></article>)}</div><h2>Misadventures</h2>{history.slice(0,25).map((h,i)=><article className="encounter" key={i}><a href={'/cell/'+h.x+'/'+h.y}>{h.x} / {h.y}</a><p>{JSON.parse(h.value).event.text}</p></article>)}<div className="passage-buttons">{page>0&&<a href={'?page='+(page-1)}>← Newer entries</a>}{history.length>25&&<a href={'?page='+(page+1)}>Older entries →</a>}</div></main>;
}

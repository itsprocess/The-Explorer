...
export default async function ProfilePage({params,searchParams}:{params:Promise<{id:string}>;searchParams:Promise<{page?:string}>}){
 ...
 <details className="profile-history" open={page>0}><summary>History</summary>{history.slice(0,25).map((h,i)=><article className="encounter" key={i}><a href={'/cell/'+h.x+'/'+h.y}>{h.x}, {h.y}</a><p>{JSON.parse(h.value).event.text}</p></article>)}<div className="buttons">{page>0&&<a href={'?page='+(page-1)}>Newer</a>}{history.length>25&&<a href={'?page='+(page+1)}>Older</a>}</div></details></main>;
}

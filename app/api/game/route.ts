...
export async function POST(request:Request){try{
 ...
 if(body.action==='intro'){await db().prepare("UPDATE characters SET value=json_set(value,'$.introSeen',json('true')),revision=revision+1 WHERE id=? AND owner=?").bind(session.id,session.owner).run();return generationStream(()=>snapshot(session.owner,session.id).then(data=>({...data,canInspect:inspect})),request);}
 if(body.action==='dev_warp'||body.action==='dev_exit'||JSON.parse((await characterRow(session.owner,session.id)).value).devState){await requireOwner();return generationStream(()=>awaitGenerated(()=>devTravel(session.owner,session.id,body)).then(data=>({...data,canInspect:inspect})),request);}
 if(body.action==='give_up')return generationStream(()=>giveUp(session.owner,session.id,body.requestId).then(data=>({...data,canInspect:inspect})),request);

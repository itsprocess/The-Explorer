...
export async function GET(request:Request){try{
 ...
 const character=(await requireCharacter(request)).id;
 if(url.searchParams.has('character')&&url.searchParams.get('character')!==character)throw new AppError('History belongs to its explorer.',403);

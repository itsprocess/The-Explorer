...
export default function CommunityPanel({x,y}:{x:number;y:number}){
 ...
 <section className="community-card"><h2>World</h2><dl>{[['Locations Visited',data?.stats.locations],['Total Moves',data?.stats.moves],['Relics Uncovered',data?.stats.relics],['Total Deaths',data?.stats.deaths],['Furthest Distance',data?.stats.furthest],['Explorers',data?.stats.explorers??(data?0:undefined)]].map(([label,value])=><div key={label}><dt>{label}</dt><dd>{number(value)}</dd></div>)}</dl></section></aside>;
}

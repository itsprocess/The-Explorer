...
export function resolveOccurrences(original:Character,p:CellPackage,visit:string,selected?:number){
 ...
 const apply=(r:Outcome,label:string,yes?:string,no?:string)=>{
  ...
  if(r.kind==='relic'){c.relicClaims??=[];if(c.relicClaims.includes(key)){event.kind='revisit';say(narrative?.repeatText);return;}c.relicClaims.push(key);c.relicsLife=(c.relicsLife??0)+1;c.relicsTotal=(c.relicsTotal??0)+1;event.kind='relic';event.relic={text:event.text};return;}

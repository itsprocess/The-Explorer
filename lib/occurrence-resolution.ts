...
export function resolveOccurrences(original:Character,p:CellPackage,visit:string,selected?:number,relicClaimed=false){
 ...
 const kill=()=>{if(!c.alive)return;assertPlayerDeath(narrative?.text||t?.death);if(rescue())return;c.alive=false;c.deaths++;c.optionConsumed=[];delete c.pendingOption;delete c.pendingTransport;const lost=deathTraits(c.traits!);c.traits=lost.traits;event.stateChanges.push(...lost.changes);event.kind='death';badge('death:'+key,'death');};

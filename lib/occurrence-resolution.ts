...
export function resolveOccurrences(original:Character,p:CellPackage,visit:string,selected?:number){
 ...
 const say=(s?:string)=>{if(s)event.text=s.replaceAll('{character_name}',c.name);};
 const badge=(id:string,kind:Badge['kind'])=>{if(!c.badges.some(b=>b.id===id)){const b={id,title:t?.badgeTitle||'A story to carry',description:t?.badgeDescription||'Remembered at '+p.scene.title,kind};c.badges.push(b);event.newBadge=b.title;}};
 ...
 const apply=(r:Outcome,label:string,yes?:string,no?:string)=>{
  ...
  if(r.kind==='give'){
   r.awards.forEach((spec,i)=>{const granted=grantTrait(c.traits!,{...spec,id:key+':'+label+':'+i+':'+c.deaths,name:t?.awardName||'A gift',description:t?.awardDescription||'Received at '+p.scene.title,source:{world:p.context.seed,x:c.x,y:c.y,title:p.scene.title,visitId:visit}});c.traits=granted.traits;event.stateChanges.push(...granted.changes);});if(r.badge)badge(key+':'+label,'treasure');event.kind='acquisition';
  }
 };

...
export function errorResponse(error:unknown){const failure=publicFailure(error);return Response.json(failure,{status:failure.status,headers:{'Cache-Control':'no-store',...(failure.code==='generation_pending'?{'Retry-After':'5'}:{})}});}

...
export function compactContext(c:CellContext){return {
 coordinate:[c.x,c.y],biome:c.biome,environment:c.environment,situations:c.situations,
...
export function scenePrompt(c:CellContext,regions:Region[],details:Details,neighbors:unknown[]):Prompt{
 const event=c.event;
 return {instructions:common+` Write one canonical scene. Title: 2–5 plain physical words; proper names only for origin or exceptional sites. Description: one paragraph, 25–55 words (80 maximum), 1–3 concrete observations. Place the visitor INSIDE the interpreted biome at this coordinate. Its defining vegetation and terrain are here, not off in the distance. Use the biome as a brief backdrop, not a checklist of variables. Avoid repeating the neighboring scene’s title, opening sentence or stock phrases; choose a different concrete detail without changing the setting. Let the strongest supported civilization activity, history or variation provide the focal observation. Mention what is present; omit inventories of absent features. Ordinary ground can remain quiet. No player outcomes in permanent scenery.
EXITS: fill every required direction key in the exits object, 10–24 words each; none in main paragraph. Describe the passage naturally with one useful distinguishing feature and a brief outward view. The destination biome is committed to the immediately adjacent cell; describe entering that setting, not approaching a perpetually distant horizon. Match the shared opening and material; do not enumerate every terrain attribute. Saved neighbor shared_exit describes the SAME passage from the opposite side: preserve passage geometry/material, reverse viewpoint. Its outward scenery describes THIS cell as viewed from the neighbor: do not copy that view into the reverse exit. Use this exit’s destination biome for what lies ahead. Other neighbor facts are continuity context only. Reveal no neighboring names, occupants, interiors, events, traps, treasure or small surprises. Do not hint at secrecy. Blocked directions stay blocked for supplied reasons. Ocean crossings require supplied firm footing.
visual_brief: one sentence for imagery. continuity_facts: up to 3 short physical facts. Omitted schema fields are unused; never add them. Event fields are subjectless PAST-tense verb phrases, 5–22 words. The app prepends the visitor name. Example: "found the old well empty after the gathering." Do not write a subject, placeholder, you/your, or a standalone scene sentence. Do not repeat scenery. App chooses outcomes; never add choices or mechanics. Describe the actual incident, not the fact that an incident occurred. Never mention automatic experiences, history recording, game systems, enum values or prompt instructions. The history record IS the sentence you write.`+
 (event?` Event kind/mode/cause/outcome are binding. Interpret broad interaction categories eclectically (social, animal, natural or mechanical), without inventing absent major features. Benign need not mean dull: vary participants, action, sensory detail and outcome. Do not repeatedly settle for shifting water, rustling foliage or noticing a minor change. Give an incident a concrete miniature beginning and result. event_narrative records the supplied outcome. consumed_narrative records what the returning visitor found after a once-only event, beginning with "found", "saw" or "noticed"; every_visit may recur. The base death narration describes only the physical cause and what happened to the visitor; never mention possessions, status, protection or lost items. Death badges describe the supplied death; honors only for honor events.`:c.occurrences&&(c.occurrences.death||c.occurrences.teleport||c.occurrences.gift||c.occurrences.challenge||c.occurrences.option)?' The supplied occurrence_setup is an actual focal situation here. Incorporate it without resolving it; separate occurrence narratives own outcomes. Variation cannot suppress this incident.':' No assigned interaction: do not invent a reward, death or badge.')+

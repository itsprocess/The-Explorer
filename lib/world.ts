...
export function broadTerrain(v:Record<string,number>){return v['biome.void']?'void':v['biome.chasms']?'chasm':v['natural.ocean']?'ocean':v['biome.lakes_ponds']>0?'lake':v['civilization.inside']?'built interior':v['biome.underground']?'natural underground':v['biome.river']>0?'river':v['biome.large_foliage']>.5?'dense foliage':v['biome.exposed_rock']>.5?'exposed rock':v['biome.groundcover']>.5?'thick groundcover':'open terrain';}
export function terrainPreview(s:string,x:number,y:number){return broadTerrain(fieldworkAt(s,x,y).values);}
export function blockedReason(terrain:string){return /ocean|lake|river/.test(terrain)?'Water blocks exploration here.':/void|chasm/.test(terrain)?'An open gulf leaves no crossing.':'The biome provides no traversable route here.';}

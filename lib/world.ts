...
export function contextFor(seed:string,x:number,y:number){
 ...
 return {version:VERSION,seed,x,y,exists:exists(seed,x,y),distance:Math.hypot(x,y),protectedOrigin:safe,safeApproach:safe,connections:open,adjacentTerrain:surroundings.map(n=>({direction:n.direction,terrain:boundaryTerrain(n.values)})).filter(n=>n.terrain.length>0),ratings,regions:refs,fieldwork:stack.fields,occurrences,
 ...
 blocked:surroundings.filter(n=>!open[n.direction]).map(n=>({direction:n.direction,reason:blockedReason(n.glimpse),terrain:boundaryTerrain(n.values)})),presentFeatures:ratings.filter(r=>r.applicable&&r.value>0).map(r=>({id:r.id,name:r.name,strength:r.value})),event:null as {id:string;kind:string;mode:string;cause:string|null;deathId:string|null;entityId:string|null;outcome?:string}|null,portalDestination:occurrences?.teleport??null,

...
export async function ensureImage(p:CellPackage):Promise<LocationImage>{
 ...
 return remember(imageKey(x,y),'image',async()=>{
  ...
  const response=await fetch('https://api.openai.com/v1/images/generations',{method:'POST',headers:{Authorization:'Bearer '+key,'Content-Type':'application/json'},body:JSON.stringify({model,prompt,n:1,size:'1536x1024',quality:'low',output_format:'webp'}),signal:AbortSignal.timeout(180000)});

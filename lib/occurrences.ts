...
export type Occurrences={lock?:{rarity:import('./traits').Rarity;kind:'tile'|'object'};relic?:boolean;death:boolean;teleport:{x:number;y:number}|null;gift:Extract<Outcome,{kind:'give'}>|null;challenge:Extract<Outcome,{kind:'challenge'}>|null;option:{legacyAutomatic?:boolean;policy:'visit'|'life'|'character';choices:Outcome[];bonusRequirement?:Requirement}|null};
...
export function occurrencesFor(seed:string,x:number,y:number,v:Record<string,number>,fields:{id:string;enumId?:string}[]):Occurrences{
 ...
 const requirement=(key:string,bonus=false,automatic=false):Requirement=>{
  ...
  if(automatic||!bonus&&random(seed,key+'trait-check',x,y)<.25)return {kind:'defining',value:pick(key+'trait',definingTraits)};
 ...
 const challenge=(key:string):Extract<Outcome,{kind:'challenge'}>=>{const r=requirement(key,false,key==='challenge'),loss=shift(key+'failure');if(loss)loss.delta=-Math.abs(loss.delta);return {kind:'challenge',requirement:r,present:success(key+'yes',r,!key.startsWith('option-')),absent:{kind:random(seed,key+'fatal',x,y)<.18?'kill':'none',standing:loss}};};

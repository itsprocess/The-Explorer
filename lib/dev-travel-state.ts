...
export function previewResult(real:Character,simulated:Character,event:{text:string;kind:string;newBadge:string|null}){
 ...
 devState.standings=structuredClone(real.standings??[]);devState.badges=structuredClone(real.badges);devState.deaths=real.deaths;devState.furthest=real.furthest;

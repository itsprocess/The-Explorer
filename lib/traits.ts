...
export function stateRuleFor(seed:string,x:number,y:number,event:{kind:string;cause?:string|null}|null,v:Record<string,number>):StateRule|null{
 ...
 if(roll<.22){
  ...
  return {kind:'grant',spec:{kind:'status',family,value:choose(seed,'status-value',x,y,statusFamilies[family] as readonly string[]),lifetime:family==='death_protection'?'single_use':lifetime}};
 }

...
export async function tileMemory(x:number,y:number){
 ...
 const relics=(await db().prepare("SELECT v.id,v.character,json_extract(c.value,'$.name') AS name,json_extract(v.value,'$.event.relic.text') AS text FROM visits v LEFT JOIN characters c ON c.id=v.character WHERE v.x=? AND v.y=? AND json_extract(v.value,'$.event.relic.text') IS NOT NULL ORDER BY v.at,v.id LIMIT 1").bind(x,y).all<{id:string;character:string;name:string|null;text:string}>()).results;

import {encounters as configEncounters,world as configWorld} from '../explorer.config.json';
...
export function transportFor(seed:string,x:number,y:number,v:Record<string,number>){
 ...
 if(!magical&&random(seed,'transport-occurrence',x,y)>=configEncounters.transportChance*distanceMultiplier(x,y,1.7))return null;
 ...
 const limit=Math.floor((configWorld.coordinateLimit-8)/8)*8;
 let tx=Math.max(-limit,Math.min(limit,Math.round((x+dx)/8)*8)),ty=Math.max(-limit,Math.min(limit,Math.round((y+dy)/8)*8));
 if(tx===x&&ty===y)tx=x<limit?x+8:x-8;

...
import {connections,devConnections,directions,exists,LIMIT,type Direction} from './world';
...
export async function confirmTransport(owner:string,id:string,requestId:string,token:string){
 ...
 if(!valid){
  ...
  const c=structuredClone(original);if(destination){c.x=d.x;c.y=d.y;}c.alive=false;c.deaths++;delete c.pendingTransport;delete c.pendingOption;c.optionConsumed=[];c.traits=deathTraits(c.traits??[]).traits;

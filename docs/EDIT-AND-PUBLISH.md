# Current scope: local development and GitHub source

The owner has superseded the publication steps below for the Fieldwork integration. Build and test locally and push source to the same GitHub repository. Do not prepare hosting packages, synchronize compiled deployment folders, or update any public server. The older procedure below is historical context only.

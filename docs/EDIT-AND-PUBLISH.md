...
Migration 0033 is the explicitly requested adventure-economy world reset; it preserves identity and sessions. Production secrets (including DEV_PASSWORD) remain in Sites. Local setup forwards locally supplied values into ignored .dev.vars and uses explorer.config.json for provider/seed defaults.
